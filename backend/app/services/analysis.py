def calculate_disparity_ratios(records: list):
    if not records:
        return []
    ...
    for race, count in data_map.items():
        ...
        if ratio >= 1.2:
            signals.append({
                "signal_type": "disparity_ratio",
                "severity": "high" if ratio > 1.5 else "medium",
                "description": f"El grupo {race} presenta un ratio de {ratio:.2f}x en comparación con el promedio.",
